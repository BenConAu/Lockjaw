...
2. **VirtIO init is the textbook plan-apply state machine — and it's a free-form sequence of `set_status` calls.** Reset → ACK → DRIVER → negotiate → FEATURES_OK → setup_queue → DRIVER_OK is spec 3.1.1. Lockjaw's *own* `docs/architecture/patterns/plan-apply.md` was written for exactly this shape. As coded today a driver can skip a step, write `Status::DRIVER_OK` directly, or call `negotiate` before `ACK`, and the compiler shrugs. A typed builder where each method returns the next state type would make the spec unrepresentable-illegal:
   ```rust
