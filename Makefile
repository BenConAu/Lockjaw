...
USER_CRATES := user/hello user/uart-driver user/device-manager user/ramfb-driver user/display-test user/virtio-blk-driver user/fat32-server user/fat32-test user/posix-server user/cprman-driver user/clock-test user/emmc2-driver user/sleep-test user/init

.PHONY: build build-release build-user build-hash clean-all run run-release run-display run-blk objdump nm check-stack check-pointers check-vtables check-init-size check-linker-symbols test test-unit test-qemu-gicv3 test-qemu-gicv2 clean pi4 test-img
