...
| 20. Typed MMIO Substrate + Driver Regime by Construction | Done | Typed register accessors + wire DTOs from regspecs/wirespecs; `run_dma_transfer` coherence envelope with `SyncCapable`-gated `DmaRegion` construction; emmc2 ADMA2 two-envelope read with `SdhciDataCompletion` (IRQ wait + B4.1 drain); `#![deny(unsafe_code)]` on every driver; userlib root `sys_*` re-export trimmed to allowlist (forbidden syscalls unreachable from `use lockjaw_userlib::*;`); `check-driver-unsafe` xtask in `make build` backstops via syn AST + macro token scan + raw-ident normalization. Pi 4B re-validated end-to-end. |
